...
当然，除了作者样式表，一般浏览器还会存在“**用户代理样式表**”，简单来讲就是**浏览器内置了一套样式表**。
...
在确定声明值时，可能出现一种情况，那就是**声明的样式规则发生了冲突**。
...
那么接下来，如果是在**同一个源中有样式声明冲突**怎么办呢？此时就会进行样式声明的优先级比较。
...
在上面的代码中，同属于**页面作者样式**，源的重要性是相同的，此时会以**选择器的权重**来比较重要性。
...
经历了上面两个步骤，大多数的样式声明能够被确定下来。但是还剩下最后一种情况，那就是**样式声明既是同源，权重也相同**。
...
层叠冲突这一步完成后，解决了**相同元素被声明了多条样式规则究竟应用哪一条样式规则的问题**。
...
![](http://tuchuang.forclh.top/notes/202405112056622.png)
...
## 面试题
[[面试题#下面的代码，最终渲染出来的效果，a 元素是什么颜色？p 元素又是什么颜色？| 下面的代码，最终渲染出来的效果，a 元素是什么颜色？p 元素又是什么颜色？]]